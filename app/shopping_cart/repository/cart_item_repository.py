...
class CartItemRepository:
    ...
    def create_new_cart_item(self, quantity: int, product_id: str, shopping_cart_id: str):
        try:
            ...
            cart_item = CartItem(quantity=quantity, price=product.price, total_price=total_price, product_id=product_id, shopping_cart_id=shopping_cart_id)
    ...
    def get_cart_item_by_id(self, cart_id: str):
        ...
        if cart_item is None:
            raise CartItemNotFoundException(f"Product with provided ID {cart_id} not found", 400)
        return cart_item
